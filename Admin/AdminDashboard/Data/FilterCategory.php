...
if (isset($_POST['filter_cate'])) {
    ...
    $query = "SELECT c.CategoryName, p.ProductID, p.ProductName, p.CategoryID, p.Qty, p.Price, p.Import_On, p.Expired_On, p.StatusID, p.Image FROM product p
        INNER JOIN category c ON c.CategoryID = p.CategoryID WHERE p.CategoryID = $filter_cate ORDER BY p.ProductID DESC ";
} else {
